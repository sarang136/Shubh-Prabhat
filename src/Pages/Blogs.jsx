        ...
        <div className="fixed inset-0 bg-black bg-opacity-40 z-50 flex items-center justify-center px-4">
          <div className="bg-white p-6 rounded-lg w-full max-w-md">
            ...
            <form onSubmit={handleEditSubmit} className="space-y-4">
              <input type="text" value={editHeadline} onChange={(e) => setEditHeadline(e.target.value)} placeholder="Main Headline" className="w-full p-2 border rounded" />
              <input type="text" value={editSubHeadline} onChange={(e) => setEditSubHeadline(e.target.value)} placeholder="Subheadline" className="w-full p-2 border rounded" />
              <textarea value={editDescription} onChange={(e) => setEditDescription(e.target.value)} placeholder="Description" className="w-full p-2 border rounded h-24 resize-none" />
      ...
      {readModalData && (
        <div className="fixed inset-0 bg-black bg-opacity-40 flex justify-center items-center z-50">
          <div className="bg-white rounded-lg p-6 w-[90%] md:w-[60%] lg:w-[50%] max-h-[90vh] overflow-y-auto relative">
            ...
            <p className="text-base text-black leading-relaxed whitespace-pre-line">{readModalData?.Description || 'सविस्तर माहिती लवकरच उपलब्ध होईल.'}</p>
          </div>
        </div>
      )}
    </div>
